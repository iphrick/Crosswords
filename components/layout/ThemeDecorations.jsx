...
/* ─── Image-based decorations (all themes) ───────────────────────────────── */
const THEME_DECORATIONS = {
  ...
  steampunk: [
    { src: '/themes/steampunk-gears.png', alt: 'Engrenagens', style: { position: 'fixed', bottom: '-50px', right: '-50px', width: '360px', opacity: 0.12, pointerEvents: 'none', zIndex: 0, transform: 'rotate(-15deg)' } },
    { src: '/themes/steampunk-gears.png', alt: 'Engrenagens', style: { position: 'fixed', top: '-50px',   left: '-50px',  width: '300px', opacity: 0.07, pointerEvents: 'none', zIndex: 0, transform: 'rotate(165deg)' } },
  ],
  ocean: [
    { src: '/themes/ocean-waves.png', alt: 'Ondas', style: { position: 'fixed', bottom: '-40px', left: '0',       width: '100%', opacity: 0.13, pointerEvents: 'none', zIndex: 0 } },
    { src: '/themes/ocean-waves.png', alt: 'Ondas', style: { position: 'fixed', top: '-60px',    right: '-60px',  width: '380px', opacity: 0.07, pointerEvents: 'none', zIndex: 0, transform: 'rotate(180deg) scaleX(-1)' } },
  ],
  sakura: [
    { src: '/themes/sakura-branch.png', alt: 'Sakura', style: { position: 'fixed', top: '-30px',    right: '-30px',  width: '380px', opacity: 0.13, pointerEvents: 'none', zIndex: 0, transform: 'rotate(5deg)' } },
    { src: '/themes/sakura-branch.png', alt: 'Sakura', style: { position: 'fixed', bottom: '-60px', left: '-60px',   width: '340px', opacity: 0.08, pointerEvents: 'none', zIndex: 0, transform: 'rotate(185deg) scaleX(-1)' } },
  ],
  'neon-noir': [
    { src: '/themes/neon-noir-city.png', alt: 'Cidade', style: { position: 'fixed', bottom: '-20px', left: '0',       width: '100%', opacity: 0.14, pointerEvents: 'none', zIndex: 0 } },
    { src: '/themes/neon-noir-city.png', alt: 'Cidade', style: { position: 'fixed', top: '60px',     right: '-80px',  width: '400px', opacity: 0.06, pointerEvents: 'none', zIndex: 0, transform: 'scaleX(-1) rotate(180deg)' } },
  ],
  terracota: [
    { src: '/themes/terracota-pottery.png', alt: 'Ânfora', style: { position: 'fixed', bottom: '-40px', right: '-20px',  width: '340px', opacity: 0.13, pointerEvents: 'none', zIndex: 0, transform: 'rotate(-8deg)' } },
    { src: '/themes/terracota-pottery.png', alt: 'Ânfora', style: { position: 'fixed', top: '60px',     left: '-60px',   width: '280px', opacity: 0.07, pointerEvents: 'none', zIndex: 0, transform: 'rotate(10deg) scaleX(-1)' } },
  ],
};

/* ─── Floating bubble configs (emoji figurinhas temáticas) ───────────────── */
...
export default function ThemeDecorations() {
  ...
  const bubbles        = BUBBLE_CONFIGS[currentTheme] || [];

  if (imgDecorations.length === 0 && bubbles.length === 0) return null;

  return (
    <>
      {/* Bubble float keyframes */}
      <style>{`
